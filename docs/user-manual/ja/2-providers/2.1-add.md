...
## マネージド OAuth プロバイダー（Kimi / Claude Pro/Max）

v3.20.0 以降、認証センターでログインし、アカウントをカードにバインドできます。トークンはローカルの account store にだけあり、カードには書きません。

1. **設定 → OAuth 認証センター**を開く。
2. **Kimi**：デバイスコードでログイン。
3. **Claude Pro/Max**：ブラウザ PKCE（`http://localhost:54545/callback`）または Claude CLI から取り込み。ブラウザログインをキャンセルすると PKCE も中止されます。
4. 対象アプリで **+** を押し、プリセット **Kimi For Coding (OAuth)** または **Claude Pro/Max (OAuth)** を選ぶ。
5. アカウントをバインドして保存。

空の公式 Claude シードカードはプロキシ / Sidecar に使えません。接頭辞なしの公式 id と複数カードは [4.6](../4-proxy/4.6-model-routing.md)。Slug とカタログは追加/編集フォームの「ルーティング slug」「ルーティングカタログに含める」。Sidecar は [4.8](../4-proxy/4.8-sidecar.md)。
...
- マッピング表は Codex の `model_catalog_json` に書き込まれ、toml の宣言と前回成功した `/v1/models` と合成されます（上流リストのリアルタイムミラーではありません）
- リストの内容はそのまま保存され、表示名とコンテキスト長はここが優先です
- 変更またはテイクオーバー更新のあと、`/model` を更新するには **Codex の再起動が必要** です（`model_catalog_json` は起動時に読み込まれます）
