...
### Combo and Sidecar (v3.20.0)

The same proxy panel has two more blocks. They take effect after Codex / Claude takeover:

| Block | What it does | Docs |
|-------|----------------|------|
| **Combo virtual models** | Define `combo/{id}` for failover or weighted round-robin | [4.7 Combo](./4.7-combo.md) |
| **Web Search / Vision Sidecar** | Hosted search on non-official models; describe images for text-only models | [4.8 Sidecar](./4.8-sidecar.md) |

Pinning a card and official account pools: [4.6 Pick a Model by Provider](./4.6-model-routing.md).
