...
> **Versus Combo**: app-level failover switches the **whole provider card** when an *unprefixed* request fails. A `{slug}/{model}` pin does not use this queue. To fail over among specific card/model pairs, use `combo/{id}` in [4.7 Combo](./4.7-combo.md) — that path does not need this switch. Official ChatGPT also will not retry the same request on another official account.
