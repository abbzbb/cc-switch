...
Starting in v3.20.0, **after Codex / Claude takeover** the picker merges each card’s models. A `{slug}/{model}` request pins that card; two or more official-family cards use an account pool for unprefixed official ids. See [4.6 Pick a Model by Provider](./4.6-model-routing.md). Cross-card strategies: [4.7 Combo](./4.7-combo.md).
