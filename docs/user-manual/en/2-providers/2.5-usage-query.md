...
| ------------------------------ | -------------------------------------------------------------------------- | -------------------- |
| **Auto Query**                 | Claude / Codex / Gemini official subscriptions, GitHub Copilot, Codex OAuth reverse proxy, Kimi For Coding (OAuth), Claude Pro/Max (OAuth) | No (enabled by default) |
...
Starting from v3.13.0, official subscription types automatically display the quota at the bottom of the provider card after enable. v3.20.0 also auto-displays Auth Center–bound Kimi / Claude Pro/Max cards:
...
| Kimi For Coding (OAuth) | Card bound to an Auth Center Kimi account       | That account’s plan usage                 |
| Claude Pro/Max (OAuth)  | Card bound to an Auth Center Claude account     | That account’s subscription usage         |

These types share a **single, unambiguous** usage source (official or hosted subscription rate), so CC Switch calls the matching query endpoint directly.
...
- **Simplified card**: For auto-query providers, the **Health Check** and **Usage Query Config** buttons are hidden to avoid interfering with the built-in display
- **Session expired notice**: If a token fails to refresh, the card shows a yellow "Session Expired" warning (Copilot / Codex OAuth / Kimi / Anthropic)
- **Auth Center**: Settings → OAuth Auth Center also shows ChatGPT / Kimi / Anthropic usage
- **Tray**: Kimi / Anthropic managed accounts do **not** show a quota summary in the tray

Pure managed cards refresh quota in the background so multi-card pools can rank accounts ([4.6](../4-proxy/4.6-model-routing.md)). API-key Kimi For Coding is still a Token Plan template and must be enabled manually below.

---

## Manual Enable (Built-in Templates + Custom Scripts)

Besides the auto-query types above, **all other providers** (including API-key Token Plan, third-party balance queries, and various proxy services) need to have the **Usage Query** switch manually turned on in the provider card before any quota is displayed.
