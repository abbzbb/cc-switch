...
| Routing slug | `{slug}/{model}` pins this card. Save an override |
| Include in routing catalog | Off hides the card from the picker; a slug prefix still works |

Managed OAuth cards can also change the bound Auth Center account without writing the token into JSON. See [4.6](../4-proxy/4.6-model-routing.md) and [2.1 Managed OAuth](./2.1-add.md#managed-oauth-providers-kimi--claude-promax).
