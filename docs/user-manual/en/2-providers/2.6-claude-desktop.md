...
After **Needs model mapping** is enabled, Claude Desktop connects to the CC Switch local gateway. `/claude-desktop/v1/models` also returns the merged `{slug}/{model}` list (separate from Claude Code’s `anthropic/{slug}/{model}`). Pinning, pools, and Combo: [4.6](../4-proxy/4.6-model-routing.md), [4.7](../4-proxy/4.7-combo.md).
