...
| Kimi For Coding | Kimi coding-specific model (API key) |
| Kimi For Coding (OAuth) | Bind after Auth Center login; token stays off the card |
| Claude Pro/Max (OAuth) | Browser PKCE or Claude CLI import |
...
| Kimi / Kimi For Coding | Moonshot Kimi models (API key) |
| Kimi For Coding (OAuth) | Bind after Auth Center login |
...
## Managed OAuth providers (Kimi / Claude Pro/Max)

Starting in v3.20.0 you can sign in at Auth Center and bind the account to a provider card. Tokens stay in the local account store; the card never shows the full secret.

### Steps

1. Open **Settings → OAuth Auth Center**.
2. **Kimi**: click **Log in with Kimi** and complete the device-code grant.
3. **Claude Pro/Max**: click **Log in with browser** (`http://localhost:54545/callback`) or **Import from Claude CLI**. Cancelling browser login aborts PKCE. Switch the CLI account and import again to add a second account.
4. In the target app (Claude / Claude Desktop / Codex, …), click **+** and pick **Kimi For Coding (OAuth)** or **Claude Pro/Max (OAuth)**.
5. Bind the account in **Select account** (or use the default) and save.

An empty official Claude seed card (no completed login, no `sk-ant-oat…`) is **not** a proxy-safe Claude Pro/Max card and cannot be a sidecar backend.

Two or more cards of the same family on one app use the account pool for unprefixed official ids; `{slug}/{model}` still pins. See [4.6](../4-proxy/4.6-model-routing.md). Usage appears in Auth Center and on the card footer; the tray does not show Kimi / Anthropic quota. Sidecar: [4.8](../4-proxy/4.8-sidecar.md).

> These account-backed flows carry Terms of Service and account-restriction risk. Review the provider terms before use.

## Routing slug and catalog

When you add or edit a **Claude / Claude Desktop / Codex** provider, the form includes:

| Field | Meaning |
|-------|---------|
| **Routing slug** | Requests use `{slug}/{model}` to pin this card. Empty uses the provider id or a name-derived slug. Save an override so a rename does not send old sessions elsewhere. |
| **Include in routing catalog** | Off hides the card from the merged Codex / Claude Desktop picker; a slug prefix still works. |

How to pick models after takeover: [4.6](../4-proxy/4.6-model-routing.md). Do not name a provider slug `combo`.
...
- The mapping table is written into Codex's `model_catalog_json` and unioned with models declared in toml plus the last successful `/v1/models` fetch (not a live upstream mirror)
- Entries are saved exactly as listed; display names and context windows come from this table
- **Restart Codex** after edits or a takeover refresh (`model_catalog_json` is loaded at Codex startup)
