...
Added in v3.13.0, the **OAuth Auth Center** (Beta) provides unified management for third-party OAuth credentials. v3.20.0 adds Kimi and Claude Pro/Max:
...
| **Kimi For Coding**       | Device-code login; bind a `Kimi For Coding (OAuth)` card   |
| **Claude Pro/Max**        | Browser PKCE or Claude CLI import; bind a `Claude Pro/Max (OAuth)` card |

**What you can do here**:

- Log in to ChatGPT / GitHub / xAI / **Kimi** accounts via the Device Code flow
- Sign into **Claude Pro/Max** in the browser (`http://localhost:54545/callback`) or import from the current Claude CLI
- View logged-in accounts, auth status, and per-account usage for ChatGPT / Kimi / Anthropic
- Set a default account when managing multiple accounts
- Remove individual accounts or log out all accounts at once

Tokens stay in the local account store and are **not** written into the provider card. Pick the matching OAuth preset and bind the account when you add/edit a card — see [2.1 Add Provider → Managed OAuth providers](../2-providers/2.1-add.md#managed-oauth-providers-kimi--claude-promax).

Kimi: click **Log in with Kimi**, enter the device code in the browser if it is not filled automatically, and wait.

Claude Pro/Max: click **Log in with browser** for PKCE, or **Import from Claude CLI**. Switch the CLI account and import again to add a second account. Cancelling browser login aborts the local callback. An empty official Claude seed card (no login, no token) cannot be a proxy or sidecar backend.

> Auth Center and provider cards show Kimi / Anthropic usage. The tray does not show quota summaries for those two families. Idle-account pooling: [4.6](../4-proxy/4.6-model-routing.md). Sidecar: [4.8](../4-proxy/4.8-sidecar.md).
