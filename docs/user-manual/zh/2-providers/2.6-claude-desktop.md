...
开启「需要模型映射」后，Claude Desktop 会连接到 CC Switch 本地网关。`/claude-desktop/v1/models` 还会返回合并后的 `{slug}/{model}` 列表（与 Claude Code 的 `anthropic/{slug}/{model}` 分开）。钉卡、账号池、Combo 见 [4.6](../4-proxy/4.6-model-routing.md)、[4.7](../4-proxy/4.7-combo.md)。
