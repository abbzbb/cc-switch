...
| 类别                       | 范围                                                                  | 是否需要用户启用 |
| -------------------------- | --------------------------------------------------------------------- | ---------------- |
| **自动查询**               | Claude / Codex / Gemini 官方订阅、GitHub Copilot、Codex OAuth 反向代理、Kimi For Coding (OAuth)、Claude Pro/Max (OAuth) | 否（默认启用）   |
| **手动启用（内置模板）**   | Token Plan、第三方余额查询                                            | 是（见下文）     |
| **手动启用（自定义脚本）** | 未被内置模板覆盖的中转服务、私有部署、特殊 API                        | 是（见下文）     |

## 自动查询（官方订阅类）

v3.13.0 起，官方订阅类供应商在启用后会**自动**在卡片底部显示配额。v3.20.0 起，绑定认证中心的 Kimi / Claude Pro/Max 托管卡同样自动显示：
...
| Kimi For Coding (OAuth) | 绑定认证中心 Kimi 账号的卡              | 该账号套餐用量              |
| Claude Pro/Max (OAuth)  | 绑定认证中心 Claude 账号的卡            | 该账号订阅用量              |

这些类型的共同特点是**数据来源唯一且语义明确**（官方或托管订阅的使用率），因此 CC Switch 直接调用对应查询接口。

### 自动查询的交互

- **卡片底部显示**：使用百分比 + 重置倒计时，颜色随使用率变化（< 70% 绿 / 70–89% 橙 / ≥ 90% 红）
- **手动刷新**：点击卡片上的刷新图标按钮重新查询
- **卡片简化**：对自动查询类供应商，**健康检查**和**用量查询配置**按钮会被自动隐藏，避免干扰内置展示
- **会话过期提示**：如果 Token 无法刷新，卡片会显示「会话已过期」警告（Copilot / Codex OAuth / Kimi / Anthropic）
- **认证中心**：设置 → OAuth 认证中心同样显示 ChatGPT / Kimi / Anthropic 账号用量
- **托盘**：Kimi / Anthropic 托管账号**不会**在托盘菜单展示配额摘要

后台会为纯托管卡刷新额度，供多卡账号池排序（见 [4.6](../4-proxy/4.6-model-routing.md)）。API Key 版 Kimi For Coding 仍属 Token Plan，需手动启用下方模板。

---

## 手动启用（内置模板 + 自定义脚本）

除了上述自动查询的供应商，**所有其他供应商**（包括 API Key 版 Token Plan、第三方余额查询、以及各类中转服务）都需要在供应商卡片上**手动打开「用量查询」开关**后才会显示配额。
