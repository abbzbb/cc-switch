...
| Kimi For Coding | Kimi 编程专用模型（API Key） |
| Kimi For Coding (OAuth) | 认证中心登录后绑定，token 不写进卡片 |
| Claude Pro/Max (OAuth) | 浏览器 PKCE 或导入 Claude CLI |
| StepFun | 阶跃星辰 Step模型 |
| ModelScope | 魔搭社区 |
...
| Kimi / Kimi For Coding | Moonshot Kimi 模型（API Key） |
| Kimi For Coding (OAuth) | 认证中心登录后绑定 |
...
## 托管 OAuth 供应商（Kimi / Claude Pro/Max）

v3.20.0 起可以用认证中心登录，再把账号绑到供应商卡。Token 只存在本机 account store，卡片里看不到完整密钥。

### 步骤

1. 打开 **设置 → OAuth 认证中心**。
2. **Kimi**：点「使用 Kimi 登录」，完成设备码授权。
3. **Claude Pro/Max**：点「使用浏览器登录」（本机回调 `http://localhost:54545/callback`），或「从 Claude CLI 导入」。取消浏览器登录会中止 PKCE。切换 CLI 账号后再导入可加第二号。
4. 回到对应应用（Claude / Claude Desktop / Codex 等），点 **+**，选预设 **Kimi For Coding (OAuth)** 或 **Claude Pro/Max (OAuth)**。
5. 在「选择账号」里绑定刚登录的账号（或使用默认账号），保存。

空的「Claude 官方」种子卡（未完成登录、没有 `sk-ant-oat…`）**不能**当作代理可用的 Claude Pro/Max 卡，也不能当 Sidecar 后端。

同一应用下两张及以上同族托管卡时，未加前缀的官方模型会走账号池；`{slug}/{model}` 仍钉卡。见 [4.6](../4-proxy/4.6-model-routing.md)。用量出现在认证中心和卡片底部，托盘不展示 Kimi / Anthropic 配额。Sidecar 见 [4.8](../4-proxy/4.8-sidecar.md)。

> ⚠️ 这些账号型接入存在服务条款与封号风险。使用前请自行审阅对应服务商条款。

## 路由 slug 与目录

添加或编辑 **Claude / Claude Desktop / Codex** 供应商时，表单底部有：

| 字段 | 作用 |
|------|------|
| **路由 slug** | 请求用 `{slug}/{model}` 选中这张卡。留空则用供应商 id 或名称生成。建议保存覆盖值，避免改名把旧会话打到别的卡。 |
| **参与路由目录** | 关闭后，这张卡不会出现在 Codex / Claude Desktop 的合并列表里，但仍可用 slug 前缀直接请求。 |

接管后如何选模型见 [4.6 按供应商选择模型](../4-proxy/4.6-model-routing.md)。供应商 slug 不要取名 `combo`。
...
- 映射表会写入 Codex 的 `model_catalog_json`，并与 toml 里声明的模型、上次成功的 `/v1/models` 结果合并（不是上游列表的实时镜像）
- 表中条目按填写内容原样保存；显示名和上下文窗口以这里为准
- **修改或接管刷新后需要重启 Codex** 才能刷新 `/model`（`model_catalog_json` 在 Codex 启动时加载）
