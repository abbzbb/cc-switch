...
只有**官方订阅类**（Claude / Codex / Gemini 官方登录、GitHub Copilot、Codex OAuth 反向代理）以及 **Kimi For Coding (OAuth) / Claude Pro/Max (OAuth)** 会在启用后自动显示配额。**其他所有供应商**（包括 API Key 版 Token Plan 和第三方余额查询）都需要手动到供应商卡片的「用量查询」面板中打开开关并选择内置模板。详见 [2.5 用量查询](../2-providers/2.5-usage-query.md)。
...
## 模型路由、Combo 与 Sidecar（v3.20.0）

### 接管后选择器里没有 `kimi/k2`？

检查：代理是否在跑、Codex 或 Claude 接管是否打开、该卡是否勾选「参与路由目录」、slug 是否被你改过。Claude Code 里应看到 `anthropic/{slug}/{model}`。详见 [4.6](../4-proxy/4.6-model-routing.md)。

### Codex `/model` 只有每张卡当前那一个模型？

`cc-switch-model-catalog.json` 以前只投影映射表 / 当前 `model`。现在会再合并 toml 里的 `models`、官方本地种子，以及上次成功的上游 `/v1/models`（`cc-switch-model-discovery.json`）。重新打开一次 Codex 接管后**完全退出并重启 Codex**。第三方卡必须加在 Codex 页。详见 [4.6](../4-proxy/4.6-model-routing.md#codex-选择器里的目录从哪来)。

### `codex -m "kimi/k2"` 打到了当前供应商而不是 Kimi 卡？

第一段必须是**已配置的路由 slug**。未知前缀会回落到当前供应商，而不是 400。打开编辑页确认 slug 徽章。

### 为什么 `gpt-5.5` 有时换账号、写成 `main/gpt-5.5` 又不会？

未加前缀且有两张及以上官方 ChatGPT 卡时走账号池（选闲号、会话粘性）。`{slug}/gpt-5.5` 钉卡并注入该卡 token。同一请求内 Official ChatGPT **不会**跨号重试。

### Combo 返回 400 / 503？

400：没有这个 `combo/{id}`。503：Combo 在，但当前应用下没有目标能解析到卡。应用级故障转移不会接过这两个错误。见 [4.7](../4-proxy/4.7-combo.md)。

### Sidecar 搜了但中转模型没变化？

需要认证中心已登录 Claude Pro/Max 或 ChatGPT Official；官方卡自己的 hosted 搜索不会被拦截；未登录时请求保持原样。见 [4.8](../4-proxy/4.8-sidecar.md)。

### 空的 Claude 官方卡能当代理 / Sidecar 用吗？

不能。请用认证中心登录或导入 CLI 后，绑定 **Claude Pro/Max (OAuth)** 预设。

### 本仓库 GitHub Releases 的 macOS 包打不开？

部分构建未经 Apple 公证。在 Finder 中对应用**右键 → 打开**。不要把未签名包与已公证的 Homebrew cask 混为一谈。
