...
/// Cancellation-safe Open/HalfOpen/Closed snapshot.
///
/// `state` and `last_opened_at` must change under the same write lock. A client
/// abort between two separate locks used to leave `Open` + `last_opened_at =
/// None`, which `allow_request` could never recover from.
struct CircuitInner {
    state: CircuitState,
...
/// 调用方应在请求结束后把该值传回 `record_success` / `record_failure` 以更新健康统计。
/// HalfOpen 名额只由 `permit` 的 Drop 释放，避免 `record_*` 与 RAII 双减、
/// 偷走后续探测的名额。覆盖客户端取消、未走到 record_* 的路径。
#[derive(Debug)]
pub struct AllowResult {
    pub allowed: bool,
    pub used_half_open_permit: bool,
    /// Drop 释放 HalfOpen 名额；业务路径只读 bool 标志。
    #[allow(dead_code)]
    permit: Option<HalfOpenPermit>,
}

#[derive(Debug)]
struct HalfOpenPermit {
...
impl Drop for HalfOpenPermit {
    fn drop(&mut self) {
        if self
            ...
            .is_ok()
        {
            release_half_open_counter(&self.counter);
        }
    }
}

fn closed_allow() -> AllowResult {
    AllowResult {
        allowed: true,
...
impl CircuitBreaker {
    ...
    pub async fn is_available(&self) -> bool {
        ...
        match inner.state {
            ...
            CircuitState::Open => {
                ...
                let ready = open_timeout_elapsed(inner.last_opened_at, timeout_seconds);
                drop(inner);
                if ready {
                    log::info!(
    ...
    pub async fn allow_request(&self) -> AllowResult {
        ...
        match inner.state {
            CircuitState::Closed => closed_allow(),
            CircuitState::Open => {
                let timeout_seconds = self.config.read().await.timeout_seconds;
                let ready = open_timeout_elapsed(inner.last_opened_at, timeout_seconds);
                drop(inner);
                ...
                match current_state {
                    ...
                    CircuitState::Open => denied_allow(),
                }
            }
    ...
    pub async fn record_success(&self, used_half_open_permit: bool) {
        // 重置失败计数。HalfOpen 名额由 AllowResult/permit Drop 释放，
        // 这里不得再减计数，否则会和 RAII 双释放、偷走后续探测名额。
        self.consecutive_failures.store(0, Ordering::SeqCst);
        self.total_requests.fetch_add(1, Ordering::SeqCst);

        // Only probe successes that held a HalfOpen permit count toward
        // recovery. A stale Closed request that finished after the circuit
        // opened must not drive HalfOpen → Closed.
    ...
    pub async fn record_failure(&self, used_half_open_permit: bool) {
        ...
        // HalfOpen 名额由 AllowResult/permit Drop 释放，这里只更新健康统计。
        // 更新计数器
        ...
        // 检查是否应该打开熔断器
        match state {
            CircuitState::HalfOpen if used_half_open_permit => {
                // HalfOpen 探测失败，立即转为 Open。无 permit 的过期 Closed
                // 失败不得打断正在进行的探测。
                log::warn!(
    ...
    fn allow_half_open_probe(&self) -> AllowResult {
        ...
        if current < max_half_open_requests {
            AllowResult {
                ...
                permit: Some(HalfOpenPermit {
                    counter: Arc::clone(&self.half_open_requests),
    ...
    async fn transition_to_open(&self) {
        let mut inner = self.inner.write().await;
        // Timestamp first so a cancelled waiter can never observe Open
        // without a recovery clock. Open + None is also treated as immediately
        // eligible for HalfOpen as a safety net.
...
fn release_half_open_counter(counter: &AtomicU32) {
...
fn open_timeout_elapsed(opened_at: Option<Instant>, timeout_seconds: u64) -> bool {
    match opened_at {
        Some(opened_at) => opened_at.elapsed().as_secs() >= timeout_seconds,
        // Stuck Open with no clock (cancelled write) is immediately eligible.
        None => true,
    }
}
...
mod tests {
    ...
    async fn test_circuit_breaker_half_open_to_closed() {
        ...
        // 记录 2 次探测成功（必须持有 HalfOpen permit 才计入）
    ...
    #[tokio::test]
    async fn test_stale_closed_success_does_not_close_half_open() {
        let config = CircuitBreakerConfig {
            failure_threshold: 2,
            success_threshold: 1,
            ..Default::default()
        ...
        // Stale in-flight Closed success must not count as a probe.
    ...
    #[tokio::test]
    async fn test_open_without_last_opened_at_is_immediately_eligible() {
    ...
    #[tokio::test]
    async fn test_transition_to_closed_is_noop_unless_half_open() {
        let breaker = CircuitBreaker::new(CircuitBreakerConfig {
            failure_threshold: 1,
            ..Default::default()
        });
        ...
        assert_eq!(breaker.get_state().await, CircuitState::Open);
        assert!(breaker.inner.read().await.last_opened_at.is_some());
    }
    ...
    #[tokio::test]
    async fn dropping_allow_result_releases_half_open_permit() {
        let breaker = CircuitBreaker::new(CircuitBreakerConfig {
            timeout_seconds: 0,
            ..Default::default()
        });
        ...
        let fourth = breaker.allow_request().await;
        assert!(fourth.allowed);
    }

    #[tokio::test]
    async fn record_success_does_not_steal_later_probe_permit() {
        let breaker = CircuitBreaker::new(CircuitBreakerConfig {
            timeout_seconds: 0,
            success_threshold: 2,
            ..Default::default()
        ...
        assert!(
            !second.allowed,
            "record_success must not free the in-flight permit before Drop"
        );
