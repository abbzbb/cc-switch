...
fn remove_server_from_codex_fails_closed_on_invalid_toml() {
    ...
    let err = cc_switch_lib::remove_server_from_codex("srv")
        .expect_err("remove should fail instead of wiping the file");
    match err {
        cc_switch_lib::AppError::McpValidation(msg) => {
            assert!(
                msg.contains("config.toml"),
    ...
    assert_eq!(
        text, broken,
        "invalid config.toml must be left untouched on remove failure"
    );
}
